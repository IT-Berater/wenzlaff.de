/*
  ...
  Anschliessend wird es per 315 Mhz drahtlos uebermittelt.
...
void setup(void)
{
  ...
  Serial.println("Starte RFID-Wifi-Reader V. 1.0 von wenzlaff.info");
    ...
    // evl. die Nummer anpassen, oder die if abfrage loeschen
     if (result != LEER){    
      if ( checksum == data_checksum ){
        digitalWrite(led_pin, HIGH); // Flash a light to show response
        
        Serial.print(result);
        Serial.println(" OK");
        // und ueber WIFI senden
        String nr =  String(result, DEC); 
        sendString(nr, true);     
        
        digitalWrite(led_pin, LOW); // Flash a light to show response
      }
    }
